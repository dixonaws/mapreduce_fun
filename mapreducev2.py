...
def clean(str_input_dataset):
	"""
	Remove punctuation from a given an input dataset (a string)
	:param str str_input_dataset: the dataset to clean
	:return: a string with the desired replacements
	"""
...
def main():
	"""
	mapreducev2
	read: read the file into an object, returns the input dataset as a string (str_input_dataset)
	clean: remove punctuation, linebreaks, etc., returns clean input dataset as a string (str_clean_input_dataset)
	split: split the input dataset into a desired number of sub datasets, returns a list with n elements (lst_subdataset)
	map:
	reduce:
	"""
