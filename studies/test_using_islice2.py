...
def reduce(str_filename, int_startpos, int_dataset_size):
	"""
...
def main():
	...
	# here is where I would start threads for each call to reduce()
	for x in range(0, int_num_lines, int_step):
		lst_all_datasets.append(reduce(str_filename, x, (x + int_step)))

	for dict_word_count in lst_all_datasets:
