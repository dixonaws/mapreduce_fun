...
print str_clean_content
lst_words = str_clean_content.split(" ")
for word in lst_words:
	if word == "": break
	try:
		dict_word_count0[word] += 1
	except KeyError:
		dict_word_count0[word] = 1

print(json.dumps(dict_word_count0))



# now start over and iterate again (as if on an independent machine)
str_content = ""
str_clean_content=""
lst_words=[]
int_start_line = 6
...
for word in lst_words:
	if word == "": break
	try:
		dict_word_count1[word]+=1
	except KeyError:
		dict_word_count1[word] = 1

print(json.dumps(dict_word_count1))

print

lst_word_counts=[]
lst_word_counts.append(dict_word_count0)
lst_word_counts.append(dict_word_count1)
